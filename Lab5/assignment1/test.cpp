...
void display(){
	...
	while(1){
		...
		char name[20],subject[20],publish[30],grade;
		f(2);
		...
		if(option==1){
			...
			cout<<"Enter name:";cin>>name;cout<<"Enter Subject:";cin>>subject;cout<<"Enter Publish:";cin>>publish;
			T[sizeT] = new Teacher(sizeT+sizeO+sizeTy+1,name,subject,publish);
			sizeT++;
		}
		else if(option==2){
			f(100);
			if(sizeT>=10){cout<<"No space available ";continue;}
			cout<<"Enter name:";cin>>name;cout<<"Enter Grade:";cin>>grade;
			O[sizeO] = new Officer(sizeT+sizeO+sizeTy+1,name,grade);
			sizeO++;
		}
			...
			for(int i=0;i<sizeT;i++){
				...
				cout<<" Teacher Name : "<<T[i]->getName()<<"\n"<<" Teacher Subject: "<<T[i]->getSubject()<<endl<<" Publish : "<<T[i]->getPublish()<<endl<<endl;
			}
			f(2);
			for(int i=0;i<sizeO;i++){
				cout<<" Officer Code : "<<O[i]->getCode()<<endl;
				cout<<" Officer Name : "<<O[i]->getName()<<"\n"<<" Officer Grade : "<<O[i]->getGrade()<<endl<<endl;
			}
			...
			if((cd>sizeT+sizeO+sizeTy)||cd<=0){
				...
				cout<<" Teacher Name : "<<T[id]->getName()<<"\n"<<" Teacher Subject: "<<T[id]->getSubject()<<endl<<" Publish : "<<T[id]->getPublish()<<endl;
				}
				else if(fd==2){
				cout<<" Officer Code : "<<cd<<endl;
				cout<<" Officer Name : "<<O[id]->getName()<<"\n"<<" Officer Grade : "<<O[id]->getGrade()<<endl<<endl;
				}
				else if(fd==3){
